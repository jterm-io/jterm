...
class DefaultTextGUITest {
    ...
    @Test
    @DisplayName("updateScreen shows real cursor when TextBox is focused")
    void updateScreenShowsCursorForFocusedTextBox() throws IOException {
        var mockTerm = new MockTerminal(new TerminalSize(80, 24));
        ...
        gui.addWindow(window);
        gui.updateScreen();
        assertNotNull(mockTerm.getCursorVisible());
        assertTrue(mockTerm.getCursorVisible(), "cursor should be visible when TextBox is focused");
    }

    @Test
    @DisplayName("updateScreen hides real cursor when no TextBox is focused")
    void updateScreenHidesCursorWhenNoTextBoxFocused() throws IOException {
        var mockTerm = new MockTerminal(new TerminalSize(80, 24));
        var screen = new DefaultScreen(mockTerm);
        var gui = new DefaultTextGUI(screen);
        var window = new WindowImpl("Test");
        window.setHints(List.of(WindowHint.FULLSCREEN));
        window.getContents().setLayoutManager(new LinearLayout(LinearLayout.Direction.VERTICAL));
        window.getContents().addComponent(new Button("Click Me"));
        gui.addWindow(window);
        gui.updateScreen();
        assertNotNull(mockTerm.getCursorVisible());
        assertFalse(mockTerm.getCursorVisible(), "cursor should be hidden when Button (not TextBox) is focused");
    }

    @Test
    @DisplayName("updateScreen positions cursor at TextBox text cursor location")
    void updateScreenPositionsCursorAtTextBoxCursor() throws IOException {
        var mockTerm = new MockTerminal(new TerminalSize(80, 24));
        var screen = new DefaultScreen(mockTerm);
        var gui = new DefaultTextGUI(screen);
        var window = new WindowImpl("Test");
        ...
        window.getContents().addComponent(textBox);
        gui.addWindow(window);
        // First render to lay out the TextBox
        gui.updateScreen();
        // Now type characters to move the text cursor
        textBox.handleKeyStroke(new KeyStroke(KeyType.CHARACTER, 'h', false, false, false));
        textBox.handleKeyStroke(new KeyStroke(KeyType.CHARACTER, 'i', false, false, false));
        gui.requestRefresh();
        gui.updateScreen();
        // Cursor should be visible and positioned at the TextBox's global position + cursor offset
        assertTrue(mockTerm.getCursorVisible());
    }

    @Test
    @DisplayName("updateScreen hides cursor after TextBox loses focus")
    void updateScreenHidesCursorAfterFocusLeavesTextBox() throws IOException {
        var mockTerm = new MockTerminal(new TerminalSize(80, 24));
        var screen = new DefaultScreen(mockTerm);
        var gui = new DefaultTextGUI(screen);
        var window = new WindowImpl("Test");
        ...
        gui.addWindow(window);
        gui.updateScreen();
        assertTrue(mockTerm.getCursorVisible(), "cursor visible when TextBox focused");
        // Tab to the button
        gui.processInput(new KeyStroke(KeyType.TAB, '\0', false, false, false));
        gui.updateScreen();
        assertFalse(mockTerm.getCursorVisible(), "cursor hidden when focus moves to Button");
    }
